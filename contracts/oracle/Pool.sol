...
contract Pool is PoolSetters, Liquidity {
    ...
    function deposit(uint256 value) external onlyFrozen(msg.sender) notPaused {
        ...
        uint256 fourPercentFee = value.mul(4).div(100);
        uint256 realDepositAmount = value.sub(fourPercentFee);

        // treasury receives fee
        univ2().transfer(Constants.getTreasuryAddress(), fourPercentFee);

        incrementBalanceOfStaged(msg.sender, realDepositAmount);

        balanceCheck();

        emit Deposit(msg.sender, realDepositAmount);
    }
    ...
    function emergencyPause() external onlyDev {
        pause();
    }
    ...
    modifier onlyDev() {
        Require.that(msg.sender == dev(), FILE, "Not dev");
