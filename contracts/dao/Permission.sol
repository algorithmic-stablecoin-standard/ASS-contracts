...
contract Permission is PoolSetters {
    bytes32 private constant FILE = "Permission";

    // Can modify account state
    modifier onlyFrozenOrFluid(address account) {
        Require.that(statusOf(account) != PoolAccount.Status.Locked, FILE, "Not frozen or fluid");
    ...
    modifier onlyFrozenOrLocked(address account) {
        Require.that(statusOf(account) != PoolAccount.Status.Fluid, FILE, "Not frozen or locked");
